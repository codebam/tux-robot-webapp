<script lang="ts">
	...
	type RenderToken = Snippet<[Token]>;

	let { token, renderToken }: { token: TableToken; renderToken: RenderToken } = $props();
	...
	const labels = $derived(token.rows.map((row) => row[0]?.text || ''));
		...
		<div class="chart-header-actions">
			...
			{#if activeView !== 'table'}
				<div class="chart-legend-row">
					{#each numericSeries as series, sIdx (sIdx)}
						<span class="legend-item">
							<span class="legend-dot" style="background-color: {colors[sIdx % colors.length]};"
							></span>
							{series.name}
						</span>
					{/each}
	...
	{:else}
		...
		<div class="chart-wrapper">
			<svg viewBox="0 0 {viewBoxWidth} {viewBoxHeight}" class="interactive-svg-chart">
				<defs>
					{#each numericSeries as _series, sIdx (sIdx)}
				...
				{#each labels as label, i (i)}
					{@const x =
						activeView === 'bar' ? paddingLeft + i * barGroupWidth + barGroupWidth / 2 : getX(i)}
					<text
						{x}
				...
				{#if activeView === 'bar'}
					{#each labels as _label, pIdx (pIdx)}
				...
				{#if activeView === 'line'}
					...
					{#each numericSeries as series, sIdx (sIdx)}
						...
						{@const zeroY = getY(Math.max(0, yBounds.min))}
						<path
							d="M {getX(0)} {zeroY} L {points.map((pt) => `L ${pt}`).join(' ')} L {getX(
					...
					<!-- Interactive Points -->
					{#each numericSeries as series, sIdx (sIdx)}
						{#each series.values as val, pIdx (pIdx)}
							{@const cx = getX(pIdx)}
							{@const cy = getY(val)}
							<circle
								{cx}
			...
			{#if hoverPoint && numericSeries[hoverPoint.sIdx]}
				...
				<div
					class="chart-tooltip"
					style="left: {(hoverPoint.x / viewBoxWidth) * 100}%; top: {((hoverPoint.y - 10) /
						viewBoxHeight) *
						100}%;"
				>
					...
					<div class="tooltip-row">
						...
						<span class="tooltip-val" style="color: {colors[hoverPoint.sIdx % colors.length]};"
							>{rawVal}</span
						>
					</div>
				</div>
